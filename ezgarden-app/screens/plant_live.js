    ...
    contents: [
	    new Line({ name: "header", left: 10, right: 10, top: 0, height: 50,
	    	contents: [
	            ...
	            new Label({ name: "label", width: 200, left: 0, right: 0, top: 0, bottom: 0, 
	            	style: plantProfile.titleText, string: "Rosemary Live" }),
	        ]
	    }),
	    new Line({ left: 20, right: 20, top: 0, height: 2, skin: plantProfile.headerLineSkin }),
	    new Picture({ name: "picture", left: 0, right: 0, top: 0, bottom: 0, aspect: "fill", url: assets.images.rosemaryLive })  
    ]
}));
...
export function refresh() {
	...
	screen.header.label.string = plant.plantType.name + " Live";
}